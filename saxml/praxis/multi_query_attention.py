...
def update_quantized_decode_state(self, name: str, new_state: JTensor) -> None:
  q_value, q_scale, _ = operations.reduce_precision(new_state, contract_dims=[-1])
  self.update_decode_state(name, q_value)
  self.update_decode_state(name + '_scale', q_scale)


def get_quantized_decode_state(self, name: str) -> JTensor:
  q_value = self.get_decode_state(name)
  q_scale = self.get_decode_state(name + '_scale')
...
class MultiQueryDotProductAttention(base_layer.BaseLayer):
  ...
  def _dot_atten_one_step(
    ...
    key = get_quantized_decode_state(self, key_state_name)
    value = get_quantized_decode_state(self, value_state_name)
    if extend_one_step:
  ...
  def _fprop_update_decode_state(self, name: str, value: JTensor) -> None:
    ...
    if (
      ...
      return
    update_quantized_decode_state(self, name, value)
  ...
  def extend_decode_state(self, name: str, value: JTensor, time_step: JTensor,
    ...
    state = get_quantized_decode_state(self, name)
    assert state is not None
    new_state = jax.lax.dynamic_update_slice(state,
                                             extend_value.astype(state.dtype),
                                             indices)
    update_quantized_decode_state(self, name, new_state)
    return new_state
