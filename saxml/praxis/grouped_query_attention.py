...
class GroupedQueryAttention(base_layer.BaseLayer):
  ...
  def _atten_context(
    ...
    # BKGTS * BSK
    logits = jnp.einsum('BKGTS,BSK->BKGTS', logits, key_scale)
    ...
    # BSKH * BSK
    value = jnp.einsum('BSKH,BSK->BSKH', value, value_scale)
  ...
  def __call__(
    ...
    k_qv, k_qs = operations.reduce_precision(k, contract_dims=[-1])
    v_qv, v_qs = operations.reduce_precision(v, contract_dims=[-1])

    self.update_decode_state('key_state', k_qv)
    self.update_decode_state('value_state', v_qv)

    self.update_decode_state('key_state_scale', k_qs)
    self.update_decode_state('value_state_scale', v_qs)
  ...
  def extend_step(
    ...
    if not is_cross_attention:
      self.extend_decode_state('key_state', k_qv, time_step)
      self.extend_decode_state('value_state', v_qv, time_step)
      self.extend_decode_state('key_state_scale', k_qs, time_step)
      self.extend_decode_state('value_state_scale', v_qs, time_step)
    encoded, atten_probs = self._atten_context(
        ...
        self.get_decode_state('key_state_scale'),
        self.get_decode_state('value_state_scale'),
