...
class MultiQueryDotProductAttention(base_layer.BaseLayer):
  ...
  def _context_for_kv_vmap(self):
    ...
    if self.activation_split_dims_mapping.blnh:
      ...
      n_sharding = None
    assert n_sharding is None or isinstance(n_sharding, str), "######n_sharding: {}, activation_sharding: {}".format(n_sharding, self.activation_split_dims_mapping)
  ...
  def __call__(
    ...
    # self._fprop_update_decode_state('key_state', key_proj.astype(jnp.int8))
    # self._fprop_update_decode_state('value_state', value_proj.astype(jnp.int8))
