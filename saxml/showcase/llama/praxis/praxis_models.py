...
class LanguageModelContinuousBatching(base_model.BaseModel):
  """Language model that uses continuous batching.
  
  To use continuous batching, the model needs to keep internal states:
    prefill_inputs: the batch inputs that are waiting for prefill
    decoding_inputs: the batch inputs that are waiting for decoding
    max_batch_size
  
  Also the model needs to implement the following API:
    prefill: run prefill on the prefill_inputs
    decode_one_step: run decoding step by step on decoding inputs
  """
  lm_tpl: LayerTpl = template_field(transformer_models.TransformerLm)
  return_predictions: bool = False
  decoder_tpl: DecoderHParams = base_layer.instance_field(GreedyDecoderHParams)
  ...
  def greedy_prefill_and_insert(
    ...
    # jax.debug.print("prefill and insert global step {x}, right aligned length {y}, per sample steps {z}", 
    #                 x=global_step, y=right_aligned_length, z=decode_state.per_sample_steps)

    decode_state.output_ids = decode_state.output_ids.at[slot_idx].set(
      decoder_utils.right_align_tensors(prefill_decode_state.output_ids, right_aligned_length)[0])
    decode_state.logprobs = decode_state.logprobs.at[slot_idx].set(
      decoder_utils.right_align_tensors(prefill_decode_state.logprobs, right_aligned_length)[0])
    
    # right aligned kv_cache in decode_state
    transformer_kv_cache = self.variables[base_layer.DECODE_CACHE]['lm']['transformer']
    for i in range(32):
      layer_kv_cache_key = 'x_layers_{}'.format(i)

      # decode_state_kv_cache = decode_state['lm']['transformer'][layer_kv_cache_key]['self_attention']
      # decode_state_kv_cache = decode_cache[base_layer.DECODE_CACHE]['lm']['transformer'][layer_kv_cache_key]['self_attention']
      decode_state_kv_cache = decode_cache['lm']['transformer'][layer_kv_cache_key]['self_attention']

      new_key_cache = transformer_kv_cache[layer_kv_cache_key]['self_attention']['key_state']
      new_value_cache = transformer_kv_cache[layer_kv_cache_key]['self_attention']['value_state']
      new_pos_emb = transformer_kv_cache[layer_kv_cache_key]['self_attention']['key_post_rotary_pos_emb']
    ...
    decode_state.done = decode_state.done.at[slot_idx].set(0) # set 0 to start decoding phase
    decode_state.has_eos = decode_state.has_eos.at[slot_idx].set(prefill_decode_state.has_eos[0])
    decode_state.decode_lengths = decode_state.decode_lengths.at[slot_idx].set(
      prefill_decode_state.decode_lengths[0])

    # update prefix infos in decode state
    decode_state.prefix_ids = decode_state.prefix_ids.at[slot_idx].set(
      prefill_decode_state.prefix_ids[0])
    decode_state.prefix_paddings = decode_state.prefix_paddings.at[slot_idx].set(
      prefill_decode_state.prefix_paddings[0])
    decode_state.prefix_lengths = decode_state.prefix_lengths.at[slot_idx].set(
      prefill_decode_state.prefix_lengths[0])

    # update input in decode_state
    decode_state.ids = decode_state.ids.at[slot_idx].set(
      prefill_decode_state.ids[0])
    decode_state.paddings = decode_state.paddings.at[slot_idx].set(
      prefill_decode_state.paddings[0])
    decode_state.per_example_max_decode_steps = decode_state.per_example_max_decode_steps.at[slot_idx].set(
      prefill_decode_state.per_example_max_decode_steps[0])
    decode_state.per_example_top_k = decode_state.per_example_top_k.at[slot_idx].set(
      prefill_decode_state.per_example_top_k[0])
    decode_state.per_example_top_p = decode_state.per_example_top_p.at[slot_idx].set(
      prefill_decode_state.per_example_top_p[0])
    decode_state.temperature = decode_state.temperature.at[slot_idx].set(
      prefill_decode_state.temperature[0])
  ...
  def greedy_init_decode_state(
    ...
    decode_state = sample_decode.greedy_init_decode_state(
        ...
        prefix_paddings=decode_data.input_paddings,
        seq_len=decode_data.seqlen,
        fprop_for_prefix=decoder_params.fprop_for_prefix,
        max_prefix_len=max_prefix_len,
        max_decode_steps=decoder_params.max_decode_steps,
        prefix_lengths=decode_data.prefix_lengths,
  ...
  def left_align_decode_state(self, decode_state):
    ...
    left_align_steps_arr = jnp.ones_like(decode_state.prefix_lengths) * left_align_steps
    # jax.debug.print("decode step global step {x}, left aligned length {y}, per_sample_steps {z}",
    #                 x=decode_state.step, 
    #                 y=left_align_steps_arr, 
    #                 z=decode_state.per_sample_steps)

    row_length = decode_state.output_ids.shape[1]
    decode_state.output_ids = jnp.where(
      decode_state.step < row_length - 1, 
      decode_state.output_ids, 
      decoder_utils.left_align_tensor(decode_state.output_ids, left_align_steps_arr, row_length-1)
    )
    decode_state.logprobs = jnp.where(
      decode_state.step < row_length - 1, 
      decode_state.logprobs, 
      decoder_utils.left_align_tensor(decode_state.logprobs, left_align_steps_arr, row_length - 1)
    )

    transformer_kv_cache = self.variables[base_layer.DECODE_CACHE]['lm']['transformer']
    # jax.debug.print("before decode_step {z} layer 0 key cache: {y}", 
    #     z=decode_state.step, 
    #     y=transformer_kv_cache['x_layers_0']['self_attention']['key_state'][0, 126, :, :])
    ...
    for i in range(32):
      ...
      new_key_cache = jnp.where(
        decode_state.step < row_length-1, 
        new_key_cache, 
        self.left_align_kv_cache(new_key_cache, left_align_steps_arr, row_length-1))
      new_value_cache = jnp.where(
        decode_state.step < row_length-1, 
        new_value_cache, 
        self.left_align_kv_cache(new_value_cache, left_align_steps_arr, row_length-1))
      new_pos_emb = jnp.where(
        decode_state.step < row_length-1, 
        new_pos_emb, 
        self.left_align_kv_cache(new_pos_emb, left_align_steps_arr, row_length-1))
      
      # if i == 0:
      #   jax.debug.print("updated decode_step {z} layer 0 key cache: {y}", 
      #       z=decode_state.step, 
      #       y=new_key_cache[0, 126, :, :])
      self.variables[base_layer.DECODE_CACHE]['lm']['transformer'][layer_kv_cache_key]['self_attention']['key_state'] = new_key_cache
      self.variables[base_layer.DECODE_CACHE]['lm']['transformer'][layer_kv_cache_key]['self_attention']['value_state'] = new_value_cache
      self.variables[base_layer.DECODE_CACHE]['lm']['transformer'][layer_kv_cache_key]['self_attention']['key_post_rotary_pos_emb'] = new_pos_emb

    # jax.debug.print("after decode_step {z} layer 0 key cache: {y}", 
    #     z=decode_state.step, 
    #     y=self.variables[base_layer.DECODE_CACHE]['lm']['transformer']['x_layers_0']['self_attention']['key_state'][0, 126, :, :])
  ...
  def greedy_process_result_with_idx(
    ...
    max_prefix_len = decode_state.ids.shape[1]
    result = sample_decode.greedy_process_result_with_idx(
        model=self.lm,
        decode_state=decode_state,
        slot_idx=slot_idx,
        fprop_for_prefix=decoder_params.fprop_for_prefix,
