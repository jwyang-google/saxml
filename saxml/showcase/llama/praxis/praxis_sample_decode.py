...
def greedy_decode_simplify(
  ...
  with decoder_utils.maybe_decode_mesh_transpose(
    ...
    # intermediate state val in decoding loop
    val = NestedMap()
    if fprop_for_prefix:
      # Update output_ids with prefix_ids.
      output_ids = jax.lax.dynamic_update_slice(
...
def greedy_decode_step_new(
  ...
  return greedy_decode_step_continuous_batching(
      model=model,
      decode_state=decode_state,
      extend_step_fn=extend_step_fn,
      prefix_ids=decode_state.prefix_ids,
      prefix_paddings=decode_state.prefix_paddings,
      seq_len=decode_state.seq_len,
      fprop_for_prefix=fprop_for_prefix,
      transform_state_fn=transform_state_fn,
      max_prefix_len=decode_state.max_prefix_len,
      max_decode_steps=max_decode_steps,
      prefix_lengths=decode_state.prefix_lengths,
      decode_loop_mesh_axes_transpose=decode_state.decode_loop_mesh_axes_transpose,
      model_var_pspecs=decode_state.model_var_pspecs,
      eos_id=decode_state.eos_id,
  )


def greedy_decode_step_continuous_batching(
    model: base_layer.BaseLayerApi,
    decode_state: NestedMap,
    extend_step_fn: (
        decoder_utils.ExtendStepFn | decoder_utils.ExpandedExtendStepFn
  ...
  with decoder_utils.maybe_decode_mesh_transpose(
    ...
    #   )
    # last_decode_steps = max(max_decode_steps) if max_decode_steps else seq_len
    last_decode_steps = max_decode_steps[0]
    per_example_max_decode_steps = last_decode_steps
...
def greedy_init_decode_state(
  ...
  if fprop_for_prefix:
    ...
    output_ids = output_ids.at[:, 0].set(prefix_ids[:, 0])
    start_step = 0
    decode_state.segment_pos = jnp.zeros([batch_size], dtype=jnp.int32)

  decode_state.start_step = start_step
  decode_state.step = start_step
  decode_state.per_sample_steps = jnp.ones(shape=batch_size, dtype=jnp.int32) * start_step
  ...
  # additional argements needed for step decoding
  decode_state['prefix_ids'] = prefix_ids
  decode_state['prefix_paddings'] = prefix_paddings
  decode_state['seq_len'] = seq_len
  decode_state['max_prefix_len'] = max_prefix_len
  decode_state['max_decode_steps'] = max_decode_steps
  decode_state['prefix_lengths'] = prefix_lengths
  decode_state['eos_id'] = eos_id
  decode_state['decode_loop_mesh_axes_transpose'] = (
      decode_loop_mesh_axes_transpose
  )
  decode_state['model_var_pspecs'] = model_var_pspecs
  decode_state['fprop_for_prefix'] = fprop_for_prefix
  return decode_state


def greedy_decode_step_with_while(
    model: base_layer.BaseLayerApi,
    decode_state: NestedMap,
    extend_step_fn: decoder_utils.ExtendStepFn
    | decoder_utils.ExpandedExtendStepFn,
  ...
  model = decoder_utils.maybe_reshard_mdl_for_decode(
      model,
      decode_state['decode_loop_mesh_axes_transpose'],
      decode_state['model_var_pspecs'],
      transform_state_fn,
  )
  with decoder_utils.maybe_decode_mesh_transpose(
      model, decode_state['decode_loop_mesh_axes_transpose']
  ):
    # decoding parameters
    max_decode_steps = decode_state.max_decode_steps
    batch_size = decode_state.prefix_ids.shape[0]
    prefix_lengths = decode_state.prefix_lengths
    eos_id = decode_state.eos_id
    ...
    # if decode_state.seq_len <= 0:
    #   raise ValueError(
    #       'The sequence length for decoding must be > 0, '
    #       f'current value = {decode_state.seq_len}.'
    #   )
    last_decode_steps = max(max_decode_steps) if max_decode_steps else decode_state.seq_len
    per_example_max_decode_steps = last_decode_steps

    # transform_fn and step_fn
    if transform_state_fn:
      # Pad to full-sequence length.
      first_max_decode_steps = (
          min(max_decode_steps)
      ...
      pad_state_sizes = (
          first_max_decode_steps if decode_state.fprop_for_prefix else decode_state.seq_len - 1
      )
      transform_state_fn(model, decoder_utils.pad_state_fn(pad_state_sizes))
    ...
    def get_cond_func(stop_at_decode_steps):
      ...
      def cond_func(model, val):
        ...
        #   (1) We have yet to exceed the max steps set by self.decoder.seqlen
        #   (2) At least one row in the batch has not terminated.
        max_steps = decode_state.start_step + stop_at_decode_steps
        length_ok = val.step < min(decode_state.seq_len - 1, max_steps)
        all_rows_done = jnp.all(val.done)
        return jnp.logical_and(length_ok, jnp.logical_not(all_rows_done))
    ...
    def loop_body(model, val):
      ...
      # Selects prefix ids when step smaller than prefix_lengths when using
      # extend_step for prefix.
      if not decode_state.fprop_for_prefix:
        new_ids = jnp.where(
            step < prefix_lengths - 1, decode_state.prefix_ids[:, step + 1], new_ids
        )
      prev_done = val.done
      new_ids = jnp.where(prev_done, jnp.zeros_like(new_ids), new_ids)
      val.output_ids = val.output_ids.at[:, step + 1].set(new_ids)
      if eos_id is not None:
        ...
        val.has_eos = jnp.logical_or(val.has_eos, has_eos)
      if decode_state.fprop_for_prefix:
        prefix_offset = decode_state.max_prefix_len
        decode_lengths = prefix_lengths + (step - decode_state.max_prefix_len + 2)
      else:
        # if eos is part of prefix, ignore it.
        val.done = jnp.where(step < prefix_lengths - 1, prev_done, val.done)
        prefix_offset = prefix_lengths
...
def greedy_process_result_with_idx(
  ...
  # logging.info("result idx: {}".format(slot_idx))
  # logging.info("process result: {}".format(decode_state))

  # process final result
  result = NestedMap()
  result.decode_lengths = jnp.expand_dims(decode_state.decode_lengths[slot_idx], axis=0)
  result.output_ids = jnp.expand_dims(decode_state.output_ids[slot_idx], axis=0)
  result.logprobs = jnp.expand_dims(decode_state.logprobs[slot_idx], axis=0)

  # logging.info("result decode lengths: {}".format(result.decode_lengths))
  # logging.info("result output ids: {}".format(result.output_ids))
  # logging.info("result logprobs: {}".format(result.logprobs))

  prefix_lengths = jnp.expand_dims(prefix_lengths[slot_idx], axis=0)
  prefix_ids = jnp.expand_dims(prefix_ids[slot_idx], axis=0)
  prefix_paddings = jnp.expand_dims(prefix_paddings[slot_idx], axis=0)
  ...
  if fprop_for_prefix:
    # Change output_ids to left align.
    left_align_max_prefix_len = decode_state.output_ids.shape[1]
    left_align_prefix_lens = jnp.expand_dims(jnp.array(
    ...
    result.logprobs = decoder_utils.left_align_tensor(
        result.logprobs, left_align_prefix_lens, left_align_max_prefix_len
    )

    # if hasattr(decode_state, 'entropy'):
    #   result.entropy = jnp.expand_dims(decode_state.entropy[slot_idx], axis=0)
    #   result.entropy = decoder_utils.left_align_tensor(
    #       result.entropy, prefix_lengths, max_prefix_len
    #   )
...
def greedy_process_result(
  ...
  # process final result
  result = decode_state
  del result.prefix_lengths, result.prefix_ids
  del result.prefix_paddings, result.seq_len
  del result.max_prefix_len, result.max_decode_steps
  del result.eos_id, result.fprop_for_prefix
  del result.decode_loop_mesh_axes_transpose, result.model_var_pspecs
  del result.lm
  del result.segment_pos
...
def greedy_decode_while_loop(
  ...
  decode_state = greedy_init_decode_state(
      model,
      prefix_ids,
      prefix_paddings,
      seq_len,
  ...
  decode_state = greedy_decode_step_with_while(
    model=model,
    decode_state=decode_state,
    extend_step_fn=extend_step_fn,
    transform_state_fn=transform_state_fn,
